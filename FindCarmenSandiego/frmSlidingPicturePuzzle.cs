...
namespace Sliding_Picture_Puzzle
{
	public partial class frmSlidingPicturePuzzle : Form
	{
		...
		private void GameWon()
		{
			...
			if (count == 9)
			{
				pic[6].Image = FindCarmenSandiego.Properties.Resources.tajmahal7;
				for (int i = 0; i < pic.Length; i++)
				{
					pic[i].Enabled = false;
				}
				MessageBox.Show("You WIN!!");
				SplashScreen_FP.SplashScreen.Brazil = true;
		...
		private void SlidePic(int num)
		{
			...
			{// blankpic[blank picture box][forloop] == picturebox clicked 
				if (blankpic[blank][i] == num + 1)
				{
					...
					pic[num].Tag = tag;
					boolBlank[num] = true;
					pic[num].Image = FindCarmenSandiego.Properties.Resources.blank;
					blank = num;
				}
			}
